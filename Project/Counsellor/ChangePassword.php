...
{
	$sel="select * from tbl_counsellor where counsellor_id='".$_SESSION["cid"]."' and counsellor_password='".$_POST["txt_cupass"]."'";
	$rowp=$Conn->query($sel);
	if($data=$rowp->fetch_assoc())
	{
		if($_POST["txt_npass"]==$_POST["txt_Conpass"])
		{
			$up="update tbl_counsellor set counsellor_password='".$_POST["txt_npass"]."' where counsellor_id='".$_SESSION["cid"]."'";
			if($Conn->query($up))
			{
				?>
                <script>
