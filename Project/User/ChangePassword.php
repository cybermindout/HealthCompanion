...
{
	$sel="select * from tbl_user where user_id='".$_SESSION["uid"]."' and user_password='".$_POST["txt_cupass"]."'";
	$rowp=$Conn->query($sel);
	if($data=$rowp->fetch_assoc())
	{
		if($_POST["txt_npass"]==$_POST["txt_Conpass"])
		{
			$up="update tbl_user set user_password='".$_POST["txt_npass"]."' where user_id='".$_SESSION["uid"]."'";
			if($Conn->query($up))
			{
				?>
                <script>
