...
{
	$sel="select * from tbl_counsiler where counsiler_id='".$_SESSION["cid"]."' and counsiler_password='".$_POST["txt_cupass"]."'";
	$rowp=$Conn->query($sel);
	if($data=$rowp->fetch_assoc())
	{
		if($_POST["txt_npass"]==$_POST["txt_Conpass"])
		{
			$up="update tbl_counsiler set counsiler_password='".$_POST["txt_npass"]."' where counsiler_id='".$_SESSION["cid"]."'";
			if($Conn->query($up))
			{
				?>
                <script>
